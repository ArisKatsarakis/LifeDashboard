...
export const WalletDisplay = () => {
	const [wallets, setWallets] = useState<Wallet.Wallet[]>([]);
	...
	return (
		<Container>
			<Row>
				...
				{wallets.map((w) => {
					return (
						<Col md={4} className="mt-2 mb-2" key={w.walletId}>
							<span key={w.walletId} className="bg-success p-2 border-2">
								<a href={`/wallet/${w.walletId}`} className="link-light">
									{" "}
									Name:{w.walletName} Pending: {w.totalPending}{" "}
								</a>
							</span>
						</Col>
					);
			...
			<Button variant="success" onClick={handleToglle}>
				{" "}
				Add Wallet{" "}
			</Button>
